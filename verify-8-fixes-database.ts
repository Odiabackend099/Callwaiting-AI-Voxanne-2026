...
async function testFix1_DashboardAnalyticsWidgets() {
  ...
  // Test 1.1: Check cost_cents column exists
...
async function testFix6_AppointmentReschedule() {
  ...
  // We can't test reschedule without modifying data, so just verify the column exists
  ...
  if (appointments && appointments.length > 0 && 'scheduled_at' in appointments[0]) {
    results.push({
      fix: 'TC010',
      test: 'appointments schema supports reschedule',
      status: 'PASS',
