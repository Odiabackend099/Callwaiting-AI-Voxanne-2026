...
CREATE POLICY "Users can view their org's hot lead alerts"
  ON hot_lead_alerts FOR SELECT
  USING (org_id = auth.org_id());

-- RLS Policy: Service role can manage all hot lead alerts
CREATE POLICY "Service role can manage hot lead alerts"
  ON hot_lead_alerts FOR ALL
  USING (true)
  WITH CHECK (true);
...
ALTER TABLE IF EXISTS integration_settings
  ADD CONSTRAINT valid_alert_phone
  CHECK (hot_lead_alert_phone IS NULL OR hot_lead_alert_phone ~ '^\+[1-9]\d{1,14}$');

-- Add comment for documentation
COMMENT ON COLUMN integration_settings.hot_lead_alert_phone IS
  'Phone number to receive SMS alerts for hot leads (E.164 format, e.g., +12345678900)';

COMMENT ON TABLE hot_lead_alerts IS
  'Tracks SMS alerts sent to clinic managers for high-value leads. Prevents duplicate alerts per call via UNIQUE constraint.';

COMMENT ON COLUMN hot_lead_alerts.lead_score IS
  'Lead scoring (0-100): 70+ is hot tier. Used to auto-trigger alerts at call end.';
