...
CREATE TABLE IF NOT EXISTS integration_settings (
  ...
  twilio_from_number TEXT,  -- E.164 format, not encrypted
  
  -- Test Configuration
  test_destination_number TEXT,  -- E.164 format, not encrypted
  ...
  CONSTRAINT valid_twilio_number CHECK (twilio_from_number IS NULL OR twilio_from_number ~ '^\+[1-9]\d{1,14}$'),
  CONSTRAINT valid_test_number CHECK (test_destination_number IS NULL OR test_destination_number ~ '^\+[1-9]\d{1,14}$')
);
