...
export async function sendDemoEmail(recipient: DemoRecipient, context: DemoContext) {
  ...
  // Send email
  const emailSubject = `Your Call Waiting AI AI Demo - ${recipient.clinic_name}`;
  const emailBody = `
Hi ${recipient.name},

Thanks for your interest in Call Waiting AI, the AI receptionist for ${recipient.clinic_name}!

Here's your personalized demo: ${demoAsset.url}

This 60-90 second video shows exactly how Call Waiting AI can:
✓ Answer 24/7 calls
...
export async function sendDemoSms(recipient: DemoRecipient, context: DemoContext) {
  ...
  const smsMessage = `Hi ${recipient.name}! Watch your Call Waiting AI AI demo here: ${demoAsset.url} - See how we capture 100% of calls 24/7!`;
