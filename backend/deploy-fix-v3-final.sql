...
BEGIN
  ...
  -- Extract service_type from metadata if available (backward compatibility)
  ...
  -- FIXED: Use c.name instead of c.first_name || ' ' || c.last_name
  ...
  -- FIXED: Use actual table schema (service_type, no notes, no metadata columns in appointments)
  ...
  -- FIXED: Use metadata column instead of event_data in audit_logs table
  INSERT INTO audit_logs (org_id, event_type, metadata, created_at)
  VALUES (
    p_org_id,
    'appointment.booked',
    jsonb_build_object(
